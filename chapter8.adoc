...
IMPORTANT: A direção de uma associação, representada pela direção da seta em um diagrama de classes, é chamada em POO de navegação. Ela define qual classe tem acesso à outra classe. Se a navegação da associação entre duas classes `A` e `B` for de `A` para `B` (`A` -> `B`), um objeto da classe `A` tem acesso a um ou mais objetos da classe `B`. Isto significa que dentro de um objeto da classe `A` pode existir um ou mais objetos da classe `B`. Se a associação for de `B` para `A` (`B` -> `A`), a classe `B` é quem terá um ou mais objetos de `A`.
